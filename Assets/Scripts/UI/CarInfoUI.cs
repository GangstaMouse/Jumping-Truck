...
public class CarInfoUI : MonoBehaviour
{
    ...
    private float GetCarTopSpeed(CarController carController)
    {
        ...
        float topAngularVelocity = (engine.MaxRPM * Engine.RPMToRad) / (gearbox.MainGear * gearbox.GearRatios[gearbox.GearRatios.Count - 1] * differential.Ratio);
        // Km/h
    ...
    private float GetCarAcceleration(CarController carController)
    {
        Engine engine = carController.GetComponent<Engine>();
        Gearbox gearbox = carController.GetComponent<Gearbox>();
        Differential differential = carController.GetComponent<Differential>();
    ...
    private float GetCarWeight(CarController carController)
    {
    ...
    public void UpdateInfo(CarController carController)
    {
    ...
    private void SetInfo(CarData carData)
    {
